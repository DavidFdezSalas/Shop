...
export const options = {
    ...
    stages: [
        { duration: '10s', target: 1 },   // Start with 10 users
        { duration: '30s', target: 2 },   // Ramp to 30 users
        { duration: '30s', target: 3 },   // Ramp to 60 users
        { duration: '30s', target: 4 },  // Ramp to 100 users
        { duration: '20s', target: 5 },  // Stay at 100 users
...
function generateUniqueUsername() {
    ...
    const vuId = __VU; // Virtual user ID
    const iteration = __ITER; // Iteration number
    return `user_${vuId}_${iteration}_${timestamp}_${randomId}`;
}
...
export default function () {
    ...
    if (success) {
        ...
        registerRate.add(1);
        console.log(`Success request - Status: ${response.status}, Body: ${response.body}`);
    } else {
